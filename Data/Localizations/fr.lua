...
|c00BE3333Utilisez d'abord le bouton « Ajouter des raiders manquants » si vous souhaitez que tout le monde obtienne des points, même ceux qui n'ont pas d'entrée boostée !]];
...
|c00%s%sg ajouté au pot par %s
Remarques]];
L.GDKP_AUCTION_DETAILS_GOLD_PAID_BY = [[
%s a payé |c00%s%sg pour
%s]];
...
|c00A79EFFMulti-Auction vous permet de lancer des enchères sur plusieurs articles à la fois, accélérant ainsi les choses !
Suivez les instructions après avoir créé cette session pour commencer

Avec |c00A79EFFSingle-Auction, vous choisissez de mettre aux enchères des objets uniques à la place ou d'utiliser la file d'attente

La sélection de |c00A79EFFMulti-Auction empêche l'ajout des éléments supprimés à la file d'attente.
Vous pouvez mélanger |c00A79EFFMulti-Auction avec |c00A79EFFSingle-Auction et la file d'attente mais nous vous le déconseillons fortement.

]];
L.GDKP_CREATE_SESSION_TYPE_LABEL = "Type d'enchère (|c00A79EFFi pour plus d'informations)";
...
L.GDKP_CUTS_ADD_RAIDER = "Ajouter un raider";
L.GDKP_CUTS_ADJUST_G = "ajuster [g]";
L.GDKP_CUTS_ADJUST_P = "ajuster [%]";
L.GDKP_CUTS_ANNOUNCE_TOOLTIP = "Annoncer la coupe de base dans le chat de groupe";
L.GDKP_CUTS_CLEAR_CONFIRMATION = "Etes-vous sûr de vouloir réinitialiser tous les joueurs et tous les calculs ? Attention : tous les joueurs qui ne participent plus au raid seront supprimés de la liste !";
L.GDKP_CUTS_DELETE_RAIDER = "Supprimer le raider";
L.GDKP_CUTS_EDIT_RAIDER = "Modifier le raider";
L.GDKP_CUTS_LOCK_TOOLTIP = "Verrouillez et payez";
...
L.GDKP_CUTS_SUMMARY = "%s Raiders | %s Avec coupe | Paiement total : %sg";
...
|c00BE3333ItemID,Minimum,Incrément|c00967FD2
18608,4000,500
]];
...
L.GDKP_LEDGER_LEGEND_BALANCE = "Solde : |c0092FF000 ? Tu es carré ! | |c00BE333330 ? vous devez %s 30g | |c00F7922E50 ? %s vous doit 50g";
...
|c00808080Il n'y a qu'un seul inconvénient : pour que les gens puissent participer à une vente aux enchères par lots, les raiders auront besoin de Gargul !
...
Utilisez le bouton |c00A79EFFFRemplir depuis l'inventaire ci-dessous pour ajouter des objets qui peuvent encore être échangés à la liste.
...
|c00A79EFF%s éléments pour les ajouter manuellement à la liste
]];
...
L.GDKP_MUTATOR_ADD = "Ajouter un mutateur";
...
L.GDKP_MUTATOR_EDIT = "Modifier le mutateur";
...
Vous avez besoin de plomb ou de butin principal pour ajuster l'or.
...
Vous avez besoin d'un butin principal ou principal pour supprimer des entrées.
...
Vous avez besoin d'un butin principal ou principal pour modifier les entrées.
...
Afficher un aperçu complet du grand livre,
...
Vous avez besoin d'un butin principal ou principal pour restaurer les entrées.
...
Les éléments seront mémorisés, même lorsque vous |c00A79EFF/reload
...
Gargul peut également gérer le pillage automatique pour vous. Découvrez-le avec |c00A79EFF/gl pm
]];
...
L.GDKP_TUTORIAL_STEP_ACTIVATE = "Activez cette session en cliquant sur le bouton |c00FFF569Enable ci-dessous !";
L.GDKP_TUTORIAL_STEP_AUCTION = "|c00FFF569%s un article pour mettre aux enchères ou mettre en file d'attente un seul article"; -- %s holds the auction item hotkey (default ALT_CLICK)
...
- |c00FFF569/gl au pour ouvrir le commissaire-priseur/la file d'attente d'un seul objet
- |c00FFF569/gl pm pour configurer le pillage automatique
...
L.GDKP_TUTORIAL_STEP_SELL = "Vous souhaitez vendre un objet sans que personne n’enchérisse ? |c00FFF569%s sur un article, choisissez un gagnant et fixez un prix !";
...
Déploiement : |c00A79EFF%s
Récompense : |c00A79EFF%s
...
L.LOCALE_CHANGE_NOTE = "Remarque : vous pouvez modifier la locale à tout moment dans les paramètres ou via |c00%s/gl locale";
...
L.LOCALE_FRFR = "F RFR"; -- French (France)
...
Êtes-vous sûr de vouloir mettre à jour vos données PlusOne existantes avec les données de %s ?
...
Dans la grande zone d'édition ci-dessous, vous pouvez fournir une liste et : inviter tout le monde, vérifier qui manque, appliquer des groupes et attribuer les chars.
...
L.RESET_SETTINGS = "Réinitialiser les options";
...
Attention : un autre jet identique a été trouvé qui peut indiquer une égalité
...
Les informations sur les réserves dures ne sont pas disponibles car les réserves logicielles
...
L.TMB_CLEAR_RAIDER_DATA = "Effacer les données du Raider";
...
L.TMB_TOOLTIP_NOTE = "|c00FFFFFF Remarque : |c00FFF569%s";
...
|c00A79EFF%s dans des sacs, des fenêtres de butin ou même sur des liens dans votre chat pour les ajouter à la file d'attente des enchères.
Vous souhaitez vendre directement un article sans enchérir ? Utilisez |c00A79EFF%s

Vous pouvez ouvrir la fenêtre %s directement en tapant |c00A79EFF/gl enchères

Gargul suit | c00FF0000TOUS les ors échangés. Vous ne voulez pas qu'un échange fasse partie de cette session GDKP ? Cochez la case « Exclure du GDKP » lors du trading !
...
|c00A79EFFOS : attribué pour hors spécification
|c00A79EFFSR : cet élément a été réservé de manière logicielle
|c00A79EFFWL : Cet article a été ajouté à la liste de souhaits (Thatsmybis)
|c00A79EFFPL : cet élément a été classé en priorité (Thatsmybis, DFT, prio3)
]];
...
|c0092FF00v%s : le lecteur est à jour
|c00F7922Ev%s : le joueur doit mettre à jour ses add-ons
|c00BE3333%s : le joueur n'a pas Gargul ou est sur une version non supportée
